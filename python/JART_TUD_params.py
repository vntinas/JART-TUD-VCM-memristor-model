...
eps=17  # from [10:25], 						# static hafnium oxide permittivity
epsphib=5.5  # hafnium oxide permittivity related to image force barrier lowering
phiBn0=0.18  # from [0.1:0.5],				# nominal schottky barrier height [eV]
phin=0.1 # from [0.1:0.3],					# energy level difference between the Fermi level in the oxide and the oxide conduction band edge [eV]
un=4e-6  # from [1e-6:1e-5],				# electron mobility [m^2/Vs]
#Ndiscmax=22  # from [0.001:1100],				# maximum oxygen vacancy concentration in the disc[10^26/m^3]
#Ndiscmin=0.004  # from [0.0001:100],			# minimum oxygen vacancy concentration in the disc [10^26/m^3]
Ninit=0.008  # from [Ndiscmin:Ndiscmax],				# initial oxygen vacancy concentration in the disc [10^26/m^3]
Nplug=20  # from [0.001:100],					# oxygen vacancy concentration in the plug [10^26/m^3]
a=0.25e-9  # from [0.1e-9:1e-9],					# ion hopping distance [m]
ny0=2e13  # from [1e10:1e14],					# attemp frequenzy [Hz]
dWa=1.35  # from [0.8:1.5],					# activation energy [eV]
Rth0=15.72e6  # from [1e6:20e6],					# thermal resistance of the Hafnium Oxide [K/W]
rdet=45e-9  # from [5e-9:100e-9],				# radius of the filament area [m]
lcell=3  # from [2:5],							# length of disc and plug region [m]
ldet=0.4  # from [0.1:5], 					# length of the disc region [m]
Rtheff_scaling=0.27  # from [0.1:1],				# scaling factor for gradual RESET
RseriesTiOx=650  # from [100:200000],			# series resistance of the TiOx layer [Ohm]
